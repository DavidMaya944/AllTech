...
public class CtrlUsuario {
	...
	public static void generarGrafic() {
		...
		for (int i = 0; i < (LogicaUsuarios.lUsuariosB.size()); i++) {
			...
			if (LogicaUsuarios.lUsuariosB.get(i).getsPermiso().equals("ACEPTADO")) {
				...
				JOptionPane.showMessageDialog(null, "No se puede cargar la grafica", "Grafica",
	...
	public static void loadData() {
		try {
			...
			JOptionPane.showMessageDialog(null, "No se ha podido cargar los datos", "Cargar datos",
					JOptionPane.ERROR_MESSAGE);
		}
	}

	public static void loadDataUser() {
		iId = Integer.parseInt(view.FrmGestionUsuarios.tableUsers
				.getValueAt(view.FrmGestionUsuarios.tableUsers.getSelectedRow(), 0).toString());
		try {
			...
			FrmDetalleUsuario.txtRol.setText(u.getRolStr());
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, "No se ha podido cargar los datos", "Cargar datos",
					JOptionPane.ERROR_MESSAGE);
		}
	...
	public void confirmarUsuario() {
		...
		JOptionPane.showMessageDialog(null, "El producto ha sido registrado con exito", "UPLOAD",
